...
export default function UsersPage() {
  ...
  const handleWithdrawRequest = async () => {
    if (!confirm("정말로 탈퇴하시겠습니까? 관리자의 승인 후 처리됩니다."))
      return;
    try {
      await axios.post(
        `${apiUrl}/users/withdraw`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  if (myRole !== "admin") {
    return (
      <AuthenticatedLayout>
        <div className="p-8 mx-auto mt-12 max-w-md text-center rounded-xl border shadow-lg bg-card">
          <ShieldAlert className="mx-auto mb-4 w-16 h-16 text-orange-500" />
          <h1 className="mb-2 text-2xl font-bold">내 프로필</h1>
          <p className="mb-6 text-muted-foreground">
            회원님의 정보 및 상태를 확인할 수 있습니다.
          </p>

          <div className="p-4 mb-8 space-y-2 text-left rounded-lg bg-muted/50">
        ...
        <div className="overflow-hidden rounded-lg border bg-card">
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              <thead className="font-medium border-b bg-muted text-muted-foreground">
                <tr>
                    ...
                    >
                      ...
                      <td className="p-4">
                        <button
                          ...
                          disabled={
                            actionLoading === user.id || user.id === myId
                          }
